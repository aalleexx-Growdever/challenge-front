          ...
          <template v-slot:top>
            ...
            <template>
              <v-btn color="success" dark class="ml-1"  @click="newUser" :disabled="isSelected">
                <span class="mdi mdi-file-plus" style="font-size: 1.5rem;" />
              </v-btn>
              <v-btn color="primary" dark class="ml-1"  @click="editUser" :disabled="!isSelected">
                <span class="mdi mdi-file-edit" style="font-size: 1.5rem;" />
              </v-btn>
              <v-btn color="error" dark class="ml-1"  @click="deleteUser" :disabled="!isSelected">
                <span class="mdi mdi-file-excel" style="font-size: 1.5rem;" />
              </v-btn>
            </template>
            ...
            <v-dialog v-model="dialogDelete" max-width="300px" persistent>
              <v-card>
                <v-card-title class="text-h6">
                  Tem a certeza de que quer apagar a matrícula?
                </v-card-title>
                <v-card-text class="text-h6">
                  {{deletingName}}
                </v-card-text>
    ...
    <v-container class="form" v-show="form" max-width="50vw" dark>
      ...
      <ValidationObserver ref="form" v-slot="{ handleSubmit }">
        <form @submit.prevent="handleSubmit(onSubmit)">
            <ValidationProvider v-slot="{ errors }" name="Course"
              rules="required" ref="form" >
              <v-select :items="courses" v-model="enrollmentForm.course"
                label="Curso" outlined :error-messages="errors"
                data-vv-name="select">
              </v-select>
            </ValidationProvider>
            <ValidationProvider v-slot="{ errors }" name="CPF"
              ...
              ref="form" >
              <v-text-field :readonly="editing" v-model="enrollmentForm.student_cpf"
              :error-messages="errors" label="CPF do aluno" required></v-text-field>
            </ValidationProvider>
          </form>
          <v-spacer></v-spacer>
...
export default {
  ...
  computed: {
    formTitle() {
      return this.editing === false ? 'Cadastrar Matrícula' : 'Editar Matrícula';
    },
    isSelected() {
      return this.selected.length !== 0;
    },
    courses() {
      return this.$store.getters.getCoursesToList;
    },
    enrollments() {
  ...
  methods: {
    ...mapActions(['requestCourses', 'requestEnrollments']),

    async initialize() {
      try {
        this.loading = true;
        await this.requestCourses();
        await this.requestEnrollments();
        this.loading = false;
      } catch {
        this.errorDialog(
    ...
    newUser() {
      if (this.areLoggedIn()) {
        this.editing = false;
        this.form = true;
        this.enrollmentForm = { ...this.defaultEnrollmentForm };
      } else {
        this.login = true;
      }
    },
    ...
    editUser() {
      if (this.areLoggedIn()) {
        this.editing = true;
        this.form = true;
        this.enrollmentForm = { ...this.selected[0] };
      } else {
        this.login = true;
      }
    },
    ...
    async confirmDelete() {
      ...
      await this.$http.delete(`/enrollments/${id}`).then(() => {
        this.successDialog(
    ...
    async onSubmit() {
      if (this.editing) {
        ...
        await this.$refs.form.validate().then((success) => {
          if (success === true) {
            this.$http.put(`/enrollments/${id}`, this.enrollmentForm)
              .then(() => {
                this.successDialog(
        ...
        await this.$refs.form.validate().then((success) => {
          if (success === true) {
            ...
            this.$http.post('/enrollments', this.enrollmentForm)
              .then(() => {
                this.successDialog(
